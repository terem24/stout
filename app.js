...
const app = {
    ...
    renderScheme: function () {
        ...
        // Генерация HTML с CSS-правилами для идеального ночного режима
        let html = `
        <style>
            #dynamic_scheme {
                position: relative; width: 100%; height: 70vh; min-height: 400px; max-height: 800px;
                background: transparent; /* Фон берется от карточки приложения */
                overflow: hidden; border-radius: 8px;
            }
            #dynamic_scheme img {
                position: absolute; top: 0; left: 0; width: 100%; height: 100%;
                object-fit: contain;
                mix-blend-mode: multiply; /* В светлой теме убирает белый фон картинок */
                transition: filter 0.3s ease, opacity 0.3s ease;
            }
            body.dark-mode #dynamic_scheme img {
                filter: invert(1) hue-rotate(180deg);
                mix-blend-mode: screen; /* В темной теме убирает инвертированный черный фон */
                opacity: 0.85; /* Слегка приглушаем яркость линий для комфорта глаз */
            }
        </style>
