...
const app = {
    ...
    addCustomEq: function () {
        if (!this.checkAccess('base')) return; 
        
        let name = prompt("Введите название оборудования:", "Новое оборудование");
        if (!name) return;
        
        let qInput = prompt("Количество:", "1");
        if (qInput === null) return;
        let q = parseFloat(qInput.replace(',', '.')) || 1;
        
        let priceInput = prompt("Цена за единицу (₽):", "0");
        if (priceInput === null) return;
        let price = parseFloat(priceInput.replace(',', '.')) || 0;

        if (!this.state.userAddedEq) this.state.userAddedEq = [];
        let id = 'custom_eq_' + Date.now();
        this.state.userAddedEq.push({ id: id, originalId: id, name: name, q: q, price: price, unit: "шт", group: "9. Своё оборудование" });
        this.saveState();
        this.render();
    },
    ...
    render: function () {
        ...
        const addToBill = (item, qty, tip, group = null) => {
            ...
            this.currentSpec.push({ ...item, q: qty, group: group });
            let finalItem = item;
            if (forceMerge) {
        ...
        // Вывод пользовательского оборудования
        if (this.state.userAddedEq && this.state.userAddedEq.length > 0) {
            this.state.userAddedEq.forEach(eq => {
                // Только крестик и имя
                let customName = `<span class="work-del-btn" style="color:#EF4444; font-weight:bold; cursor:pointer; margin-right:8px; font-size:14px;" onclick="event.stopPropagation(); app.deleteEq('${eq.id}')" title="Удалить">✖</span>${eq.name}`;
                
                // Передаем пустой бренд
                let customEqItem = { ...eq, name: customName, brand: "" };
                addToBill(customEqItem, eq.q, "", "9. Своё оборудование");
            });
            flushBill("9. Своё оборудование");
        }

        // Кнопка добавления своего оборудования (только на вкладке оборудования)
        if (this.state.viewMode === 'equipment') {
            let btnClassEq = (!this.state.tgUser) ? "btn-add-zone locked-guest" : "btn-add-zone";
            h += `<tr class="hide-custom-eq-btn" style="background: transparent; border: none;"><td colspan="9" style="text-align: center; padding: 25px 0;"><button class="${btnClassEq}" style="max-width:300px; margin:0 auto; font-size: 14px; padding: 12px;" onclick="app.addCustomEq()">+ Добавить своё оборудование</button></td></tr>`;
        }
        // ==========================================
        ...
        // Очищаем старую схему и вставляем новую
        let oldScheme = document.getElementById('dynamic_scheme');
        if (oldScheme) oldScheme.remove();
        
        // Строгое условие: если мы на вкладке оборудования и тумблер включен (true)
        if (this.state.viewMode === 'equipment' && this.state.showScheme === true) {
            let table = document.querySelector('.inv-table');
            if (table) table.insertAdjacentHTML('afterend', this.renderScheme());
        }
        this.saveState();
