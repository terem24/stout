...
const app = {
    ...
    toggleScheme: function (chk, event) {
        if (!this.checkAccess('pro', event)) {
            // Если нет прав, возвращаем тумблер в выключенное состояние
            document.getElementById('chk_scheme').checked = false;
            return;
        }
    ...
    render: function () {
        ...
        // Вывод пользовательского оборудования (Group "9. Своё оборудование")
        if (this.state.userAddedEq && this.state.userAddedEq.length > 0) {
            this.state.userAddedEq.forEach(eq => {
                // Оставляем только крестик и имя
                ...
                // ВАЖНО: Явно добавляем свойство brand: "" прямо в объект
