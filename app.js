...
const app = {
    ...
    render: function () {
        ...
        // 1. Обязательно удаляем старый контейнер схемы, чтобы не было дублей
        let oldScheme = document.getElementById('dynamic_scheme');
        if (oldScheme) oldScheme.remove();

        // 2. Проверяем условие: вкладка оборудования И тумблер включен
        if (this.state.viewMode === 'equipment' && this.state.showScheme) {
            
            // 3. Ищем целевую таблицу. Если класса .inv-table нет, берем первый попавшийся тег table в рабочей зоне
            let tableTarget = document.querySelector('.inv-table') || document.querySelector('.content-area table') || document.querySelector('table');
            
            if (tableTarget) {
                // Вставляем схему ПЕРЕД таблицей спецификации
                tableTarget.insertAdjacentHTML('beforebegin', this.renderScheme());
            } else {
                console.warn("Таблица спецификации не найдена, схема не может быть отрисована.");
            }
        }
        this.saveState();
...
// Автоматическое отключение тёмной темы и перенос схемы ВНИЗ перед печатью
...
// Возврат тёмной темы и возврат схемы НАВЕРХ после печати
