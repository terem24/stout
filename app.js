...
const app = {
    ...
    syncUI: function () {
        ...
        if (document.getElementById('chk_group')) document.getElementById('chk_group').checked = this.state.groupItems;
    ...
    toggleSku: function (event) {
        if (!this.checkAccess('pro', event)) {
            let el = document.getElementById('chk_sku');
            if (el) el.checked = this.state.showSku;
            return;
        }
        setTimeout(() => {
            let el = document.getElementById('chk_sku');
            if (el) this.state.showSku = el.checked;
    ...
    toggleScheme: function (chk, event) {
        if (!this.checkAccess('pro', event)) {
            let el = document.getElementById('chk_scheme');
            if (el) el.checked = this.state.showScheme;
            return;
        }
        this.state.showScheme = chk;
        this.render();
    },
    toggleGrouping: function (chk) {
        this.state.groupItems = chk;
        this.render();
    },
    ...
    render: function () {
        ...
        if (this.state.hotWater) {
            ...
            // БЕЗОПАСНЫЙ ПОДБОР БОЙЛЕРА (без циклов)
            let searchVol = parseInt(targetVol) || 0; // Защита от NaN (если поле ввода пустое, считаем как 0)
            
            let tankDb = (this.state.boilerType === 'optibase') ? catalog.tanks_optibase : catalog.tanks_standard;
            let tank = tankDb.find(t => t.vol >= searchVol); // Находим первый подходящий бойлер за долю секунды

            let boilerWarn = "";
            if (!tank) {
                // Если нужного объема нет (например, требуется 600л), берем самый большой доступный бойлер (последний в массиве)
                tank = tankDb[tankDb.length - 1]; 
                
                if (searchVol > 500) {
                    boilerWarn = '⚠️ Требуемый объем ГВС превышает 500л! Добавьте в смету второй бойлер вручную или проверьте количество потребителей ГВС.';
                    if (app.tempWarns) app.tempWarns.push(boilerWarn);
                }
            }
            
            tank.alts = [catalog.tanks_optibase[0], catalog.tanks_standard[0]];
            addToBill(tank, 1, this.getDesc('boiler_tank', this.state.res, tank.vol, hw_fixtures_vol));
        }
        flushBill("1. Котёл + водонагреватель", boilerWarn);
