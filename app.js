...
const app = {
    ...
    renderScheme: function () {
        ...
        // 4. Блок Бойлера (ГВС) и Водоснабжения
        if (s.hotWater) {
            ...
            // Комплекты Fugas (проверяем наличие фугаса в смете)
            if (hasItem("Fugas") || hasItem("fugas")) {
        ...
        // Генерация HTML с обработчиком ошибок загрузки картинок, выравниванием и ночным режимом
        let bgStyle = s.darkMode ? 'background: transparent;' : 'background: #fff;';
        let imgFilter = s.darkMode ? 'filter: invert(0.85) hue-rotate(180deg); opacity: 0.9;' : '';

        let html = `<div id="dynamic_scheme" style="position: relative; width: 100%; height: 70vh; min-height: 400px; max-height: 800px; ${bgStyle} overflow: hidden; transition: background 0.3s ease; border-radius: 8px;">`;

        layers.forEach(layer => {
            // Если это слой с текстом - прижимаем влево, если оборудование - вправо
            let position = (layer === 'bg_frame.png') ? 'left center' : 'right center';

            html += `<img src="${basePath}${layer}" alt="${layer}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: contain; object-position: ${position}; transition: filter 0.3s ease; ${imgFilter}" onerror="this.style.display='none'">`;
        });
