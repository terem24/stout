...
const app = {
        ...
        try {
            ...
            if (tgUser && (tgUser.id || tgUser.email)) {
                ...
                if (uData) {
                    ...
                    let { data: newUser, error: insertError } = await supabaseClient.from('users').insert([insertObj]).select();
                    if (newUser && newUser[0]) dbUserId = newUser[0].id;
                }
            }
    ...
    sendEmail: async function () {
        ...
        try {
            ...
            // TODO: Замените на ваши реальные ID из личного кабинета EmailJS
            const EMAILJS_SERVICE_ID = "service_stout"; 
            const EMAILJS_TEMPLATE_ID = "template_estimate";
            const EMAILJS_PUBLIC_KEY = "user_placeholder"; 

            // Сборка текста спецификации для письма
            let equipmentText = "";
            (this.currentSpec || []).forEach((item, idx) => {
                equipmentText += `${idx + 1}. ${item.name} — ${item.q} ${item.unit || 'шт'} (${((item.price || 0) * item.q).toLocaleString('ru-RU')} ₽)\n`;
            });

            const templateParams = {
                project_name: this.state.projectName || "Без названия",
                user_name: tgUser.first_name || tgUser.username || "Монтажник",
                user_phone: tgUser.phone || "",
                user_email: tgUser.email || "",
                area: this.state.area || 0,
                eq_sum: (app.lastEqSum || 0).toLocaleString('ru-RU') + " ₽",
                works_sum: (app.lastWorksSum || 0).toLocaleString('ru-RU') + " ₽",
                total_sum: ((app.lastEqSum || 0) + (app.lastWorksSum || 0)).toLocaleString('ru-RU') + " ₽",
                equipment_list: equipmentText,
                manager_email: "kovdor24@yandex.ru"
            };

            // Отправка через EmailJS SDK
            await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams, EMAILJS_PUBLIC_KEY);
            
            alert("✅ Смета успешно сохранена в облаке и отправлена менеджеру!");
            
        } catch (error) {
            console.error("Email send error:", error);
            // Обработка ошибок без парсинга JSON (EmailJS возвращает объект с полем text)
            let errorMsg = "Не удалось отправить письмо.";
            if (error && error.text) errorMsg += " Причина: " + error.text;
            else if (error && error.message) errorMsg += " Причина: " + error.message;
            
            alert("⚠️ Смета сохранена в облаке, но возникла ошибка при отправке Email. " + errorMsg);
        } finally {
            btn.innerHTML = originalHtml;
            btn.disabled = false;
        }
