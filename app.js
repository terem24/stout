...
window.addEventListener('beforeprint', function () {
    ...
    // Запоминаем текущее состояние, чтобы вернуть его после печати
    let originalMode = app.state.viewMode;
    let printArea = document.getElementById('print-area');
    let originalScheme = document.getElementById('dynamic_scheme');

    if (printArea) {
        // --- ШАГ А: ЛИСТ ОБОРУДОВАНИЯ ---
        app.state.viewMode = 'equipment';
        app.render(); 
        let eqClone = printArea.cloneNode(true);
        eqClone.id = 'print_eq_clone';
        // Вырезаем схему из середины листа, чтобы она не дублировалась
        let eqScheme = eqClone.querySelector('#dynamic_scheme');
        if (eqScheme) eqScheme.remove(); 
        printBin.appendChild(eqClone);

        // --- ШАГ Б: ЛИСТ МОНТАЖНЫХ РАБОТ (Если есть PRO) ---
        let isPro = app.state.tgUser && ['pro', 'admin'].includes(app.state.tgUser.account_type);
        if (isPro) {
            app.state.viewMode = 'works';
            app.render();
            let worksClone = printArea.cloneNode(true);
            worksClone.id = 'print_works_clone';
            worksClone.classList.add('print-page-break'); // Разрыв страницы перед монтажом
            let worksScheme = worksClone.querySelector('#dynamic_scheme');
            if (worksScheme) worksScheme.remove(); 
            printBin.appendChild(worksClone);
        }

        // --- ШАГ В: СХЕМА (На отдельном листе в самом конце) ---
        if (app.state.showScheme && originalScheme) {
            // Находим актуальную перерисованную схему
            let currentScheme = document.getElementById('dynamic_scheme');
            if (currentScheme) {
                let schemeClone = currentScheme.cloneNode(true);
                // Оставляем оригинальный ID, чтобы сработали стили альбомной ориентации
                printBin.appendChild(schemeClone);
            }
        }

        // --- ШАГ Г: ПРЯЧЕМ ОРИГИНАЛ ОТ ПРИНТЕРА ---
        ...
        // --- ШАГ Д: ВОЗВРАЩАЕМ ИНТЕРФЕЙС КАК БЫЛО ---
        app.state.viewMode = originalMode;
        app.render();
    }
});

// Уборка и возврат к нормальной жизни после закрытия окна печати
