    ...
    render: function () {
        ...
        const flushBill = (title, warn) => {
            ...
            bill.forEach((i, arrIndex) => {
                ...
                let descText = i.desc ? i.desc : (i.qtyTip ? i.qtyTip.replace(/<[^>]*>?/gm, '') : '');
                let finalTitle = `${descText}${availText}`.trim().replace(/"/g, '&quot;');
                let tipHtml = finalTitle ? `<i class="info-icon" style="${availStyle}" title="${finalTitle}">i</i>` : "";
