        ...
        const app = {
            ...
            saveToCloud: async function () {
                ...
                const total = (app.lastEqSum || 0) + (app.lastWorksSum || 0);
                // Пытаемся получить данные пользователя, если открыто в Telegram
                const tgUser = (window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.initDataUnsafe && window.Telegram.WebApp.initDataUnsafe.user) ? window.Telegram.WebApp.initDataUnsafe.user : this.state.tgUser;

                try {
                    let dbUserId = null;
                    if (tgUser && (tgUser.id || tgUser.email)) {
                        // Ищем пользователя в нашей базе Supabase
                        ...
                        if (uData) {
                            ...
                            // Если монтажник зашел впервые - создаем ему профиль
                    ...
                    const insertData = { project_name: pName, calc_data: this.state, total_sum: total };
            ...
            syncUI: function () {
                ...
                if (authContainer) {
                    ...
                    if (tgUser) {
                        ...
                        // ПРОВЕРКА НА АДМИНА
                        let adminEmails = ['kovdorekb@gmail.com', 'kovdor24@yandex.ru'];
            ...
            render: function () {
                ...
                if (this.state.water && this.state.waterZones.length > 0) {
                    let mainTitle = "5. Водоснабжение";
                    let isMerge = this.state.mergeItems;
                    let grpCold = isMerge ? mainTitle : "5.1. Внутреннее ХВС";
                    ...
                    if (totalToilets > 0) {
                        ...
                        addToBill(catalog.water_parts[6], totalToilets, this.getDesc('install'), "6. Канализация");
                        flushBill("6. Канализация");
                    }

                    // === 5.5 СКВАЖИНА ===
                    if (this.state.well) {
                        let grpWell = "5. Внешнее водоснабжение";
                        let q = 0;
                        ...
                        // Логика подбора ROMMER: Напор насоса (Hmax) должен быть больше расчетного на 25-30м для нормальной рабочей точки. Qmax должен быть больше или равен расчетному.
                        ...
                        let grpWellTie = "5.1. Обвязка скважинного насоса"; // ИСПРАВЛЕНО: передаем группу как строку для корректного парсинга
                ...
                if (cl) {
                    if (cl.type === 'pro65') {
                        ...
                        addToBill(p1, Math.ceil(vP / p1.vol), "Концентрат.", "7. Дополнительные материалы"); addToBill(p2, Math.ceil(vH / p2.vol), "Вода.", "7. Дополнительные материалы");
                    } else {
                        addToBill(cl, Math.ceil(vSys / cl.vol), this.getDesc('coolant', Math.round(vSys)), "7. Дополнительные материалы");
                    }
                }
                flushBill("7. Дополнительные материалы");
                // ==========================================
