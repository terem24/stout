    ...
    renderZonesUI: function () {
        ...
        this.state.waterZones.forEach((z, idx) => {
            ...
            for (let [key, name] of Object.entries(labels)) {
                ...
                itemsHtml += `<div class="zone-row">
                    <span class="fixture-label">${name}</span>
                    <div class="stepper">
                        <button class="step-btn" onclick="app.updZoneFixture(${z.id}, '${key}', -1)">-</button>
            ...
            let html = `<div class="zone-card">
                <div class="zone-header">
                    <span class="zone-title" contenteditable="true" onblur="app.state.waterZones[${idx}].name=this.innerText">${z.name}</span>
                    <div class="zone-remove" onclick="app.removeZone(${z.id})">×</div>
                </div>
                <div class="zone-row" style="margin-bottom:12px; padding-bottom:8px; border-bottom:1px dashed var(--border);">
                    <span class="fixture-label">Трасса до объекта (м)</span>
                    <input type="number" class="zone-input" value="${z.dist}" onchange="app.updZoneDist(${z.id}, this.value)">
                </div>
                ${itemsHtml}
            </div>`;
