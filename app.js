...
const app = {
    ...
    showProForm: function (planName) {
        ...
        document.getElementById('pro_form_block').style.display = 'block';
        document.getElementById('selected_plan').value = planName;
        document.getElementById('pro_form_title').innerText = "Оформление: " + planName;
        
        // Прячем старое сообщение об успехе, если оно было
        let msg = document.getElementById('pro_success_message');
        if (msg) msg.style.display = 'none';
        let form = document.getElementById('pro-form');
        if (form) {
            ...
            const user = this.state.tgUser || {};
            if (!form.user_name.value) form.user_name.value = user.first_name || user.name || "";
            if (!form.user_email.value) form.user_email.value = user.email || "";
        }
    },
    ...
    submitProRequest: async function (e) {
        e.preventDefault();
        ...
        try {
            ...
            const EMAILJS_TEMPLATE_ID = "template_ysuxfio"; // Новый ID шаблона
            ...
            if (res.status === 200) {
                ...
                document.getElementById('pro_form_title').innerText = "Заявка отправлена";
                form.reset();
            } else {
            ...
            alert("⚠️ Ошибка при отправке заявки. Пожалуйста, попробуйте позже.");
        } finally {
    ...
    saveToCloud: async function (silent = false) {
        ...
        let userCity = (this.state.tgUser && this.state.tgUser.city) || (this.state.user && this.state.user.city) || localStorage.getItem('user_city');
        ...
        try {
            ...
            // Используем upsert для синхронизации: если calc_id уже есть, обновим существующую запись.
            // Если calc_id нет (null), Supabase создаст новую запись.
            const { error } = await supabaseClient
                .from('estimates')
                .upsert([insertData], { onConflict: 'calc_id' });
    ...
    sendEmail: async function () {
        ...
        try {
            ...
            const templateParams = {
                ...
                user_city: tgUser.city || "Не указан",
