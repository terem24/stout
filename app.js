    ...
    render: function () {
        ...
        ['gas', 'el'].forEach(ft => {
            if (this.state.fuels.includes(ft)) {
                ...
                if (ft === 'el') {
                    if (!b && needed > 27) { let half = needed / 2; let b2 = db.find(x => x.power >= half); if (b2) { addToBill(b2, 2, this.getDesc('boiler_el', needed)); selBoilers.push(b2, b2); } }
                    else { let t = b || db[db.length - 1]; addToBill(t, 1, this.getDesc('boiler_el', needed)); selBoilers.push(t); }
                } else if (ft === 'gas') {
        ...
        if (!useEco) {
            ...
            if (needCollector) {
                ...
                if (dn25) {
                    ...
                    if (this.state.hydroType === 'combo') {
                        let item = catalog.hydro_dn20[idx]; addToBill(item, 1, "Коллектор-гидрострелка (Комби).");
                    } else {
                        let item = catalog.hydro_modular_dn20[idx]; addToBill(item, 1, "Распр. коллектор."); addToBill(catalog.hydro_arrow, 1, "Гидравлическая стрелка.");
                    }
                }
            }
            ...
            if (dn25) {
                let activePump = catalog.pumps_dn25.find(p => p.type === this.state.pumpType) || catalog.pumps_dn25[0]; pmp = activePump;
            } else {
