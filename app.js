...
window.onTelegramAuth = async function (user) {
    ...
    app.syncUI();
        ...
        const app = {
            state: { convConnectionType: 'straight', detailedRooms: false, rooms: [], convectorType: 'scq', well: true, wellDepth: 40, wellDist: 10, wellAutoType: 'sirio', h1: 2.7, h2: 2.7, viewMode: 'equipment', optItems: {}, darkMode: false, area: 150, floors: 1, region: 100, mat: 1.0, fuels: ['el'], systems: [], hotWater: false, recirc: false, res: 3, win: 4, tp1: 0, tp2: 0, showSku: false, coolant: 'water', groupItems: false, collapsedGroups: [], swaps: {}, showSwapFor: null, radType: 'space', headType: 'gas', connectionType: 'angled', boilerType: 'optibase', ufhZones: 1, ufhCtrl: 'mech', pumpType: 'default', boilerSeries: 'plus', hydroType: 'combo', pipeType: 'insulated', ufhBaseType: 'mat', radManifoldType: 'standard', water: false, waterZones: [], ufhAuto: false, projectName: "", customWorks: {} },
        ...
        let btn = document.querySelector('[onclick*="saveToCloud"]');
        if (!btn) return;
        if (this.hasUnsavedChanges) {
            btn.style.border = "2px solid #EF4444";
            let span = btn.querySelector('.unsaved-badge');
            if (!span) btn.insertAdjacentHTML('beforeend', `<span class="unsaved-badge" style="color:#EF4444; font-size:11px; font-weight:700; margin-left:5px;">(не сохр.)</span>`);
        } else {
            btn.style.border = "";
            let span = btn.querySelector('.unsaved-badge');
            if (span) span.remove();
    ...
    setProjectName: function (val) {
        ...
        this.state.projectName = val.trim(); this.saveState();
    },
    ...
    filterAdminData: function (query) {
        ...
        rows.forEach(row => {
            ...
            renderScheme: function() {
                let s = this.state;
                let html = `<div id="dynamic_scheme" style="padding: 20px; background: var(--surface); border: 1px solid var(--border); border-radius: 12px; margin-bottom: 20px; overflow-x: auto; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
                    <h3 style="margin-top: 0; margin-bottom: 15px; font-size: 16px; color: var(--text-main);">📊 Тепломеханическая схема (Авто-генерация)</h3>
                    <div style="display: flex; align-items: center; gap: 15px; min-width: 650px; font-size: 12px; text-align: center;">`;

                // 1. Котел
                let boilerBg = s.fuels.includes('el') ? '#FEF3C7' : '#FEE2E2';
                let boilerBorder = s.fuels.includes('el') ? '#F59E0B' : '#EF4444';
                let boilerName = s.fuels.includes('el') ? '⚡ Электрический<br>котел' : '🔥 Газовый<br>котел';
                html += `<div style="border: 2px solid ${boilerBorder}; padding: 15px; border-radius: 8px; width: 100px; background: ${boilerBg}; color: var(--text-main); font-weight: bold; flex-shrink: 0;">${boilerName}</div>`;

                html += `<div style="height: 4px; background: var(--border); width: 30px; flex-shrink: 0;"></div>`;

                // 2. Гидрострелка
                let hasManySystems = (s.systems.includes('rad') && s.systems.includes('tp')) || s.hotWater;
                let hydroName = hasManySystems ? '🔀 Гидрострелка<br>и Коллектор' : '➖ Прямое<br>подключение';
                html += `<div style="border: 2px solid #6B7280; padding: 15px; border-radius: 8px; width: 100px; background: #F3F4F6; color: #374151; font-weight: bold; flex-shrink: 0;">${hydroName}</div>`;

                // 3. Контуры
                html += `<div style="display: flex; flex-direction: column; gap: 10px; flex: 1;">`;

                if (s.systems.includes('rad')) {
                    html += `<div style="display: flex; align-items: center; gap: 10px;">
                                <div style="height: 4px; background: var(--border); width: 40px;"></div>
                                <div style="border: 2px solid #EF4444; padding: 10px; border-radius: 8px; flex: 1; background: #FEF2F2; color: #991B1B;"><b>🔴 Насос (Радиаторы)</b></div>
                                <div style="height: 4px; background: var(--border); width: 40px;"></div>
                                <div style="border: 2px dashed #EF4444; padding: 10px; border-radius: 8px; flex: 1; color: var(--text-main); font-weight: bold;">Коллектор РО</div>
                             </div>`;
                }
                if (s.systems.includes('tp')) {
                    html += `<div style="display: flex; align-items: center; gap: 10px;">
                                <div style="height: 4px; background: var(--border); width: 40px;"></div>
                                <div style="border: 2px solid #10B981; padding: 10px; border-radius: 8px; flex: 1; background: #ECFDF5; color: #065F46;"><b>🟢 Насос со смесителем (ТП)</b></div>
                                <div style="height: 4px; background: var(--border); width: 40px;"></div>
                                <div style="border: 2px dashed #10B981; padding: 10px; border-radius: 8px; flex: 1; color: var(--text-main); font-weight: bold;">Коллектор ТП</div>
                             </div>`;
                }
                if (s.hotWater) {
                     html += `<div style="display: flex; align-items: center; gap: 10px;">
                                <div style="height: 4px; background: var(--border); width: 40px;"></div>
                                <div style="border: 2px solid #3B82F6; padding: 10px; border-radius: 8px; flex: 1; background: #EFF6FF; color: #1E40AF;"><b>🔵 Насос загрузки БКН</b></div>
                                <div style="height: 4px; background: var(--border); width: 40px;"></div>
                                <div style="border: 2px dashed #3B82F6; padding: 10px; border-radius: 8px; flex: 1; color: var(--text-main); font-weight: bold;">Бойлер (ГВС)</div>
                             </div>`;
                }
    ...
    sendTg: function () {
        ...
        let text = `👋 Новый заказ с сайта STOUT Калькулятор!\n`;
        text += `━━━━━━━━━━━━━━━━━━━━━\n`;
    ...
    reset: function () {
        ...
        this.state = {
            ...
            radManifoldType: 'standard', water: false, waterZones: [], ufhAuto: false, projectName: "", customWorks: {},
    ...
    syncUI: function () {
        ...
        if (nameEdit) {
            nameEdit.innerText = this.state.projectName;
        ...
        const applyLock = (elId, reqLvl) => {
            ...
            if (elId === 'chk_dark' || elId === 'chk_water' || elId === 'chk_sku' || elId === 'chk_merge' || elId === 'chk_detailed_rooms') {
        ...
        // Синхронизация тумблера Группировать
        if (document.getElementById('chk_merge')) {
            document.getElementById('chk_merge').checked = this.state.groupItems;
        }
    ...
    renderZonesUI: function () {
        ...
        this.state.waterZones.forEach((z, idx) => {
            ...
            for (let [key, name] of Object.entries(labels)) {
                ...
                itemsHtml += `<div class="zone-row"><span style="font-size:11px;">${name}</span><div class="stepper" style="width:70px; height:24px;"><button class="step-btn" onclick="app.updZoneFixture(${z.id}, '${key}', -1)">−</button><div class="step-val" style="font-size:11px;">${val}</div><button class="step-btn" onclick="app.updZoneFixture(${z.id}, '${key}', 1)">+</button></div></div>`;
            }
    ...
    render: function () {
        ...
        const addToBill = (item, qty, tip, group = null) => {
            ...
            if (forceMerge) {
                ...
                if (existing) {
                    existing.q += qty;
                    existing.sum += finalItem.price * qty;
                    if (tip && tip.includes('|||')) {
                        let parts = tip.split('|||');
                        let locInfo = parts[0];
                        let devInfo = parts[1];
                    ...
                    bill.push({ ...finalItem, q: qty, sum: finalItem.price * qty, displaySku: finalItem.id, qtyTip: finalTip || "", group: group, originalId: item.id });
                }
            } else {
                let finalTip = tip;
        ...
        const flushBill = (title, warn) => {
            ...
            bill.forEach((i, arrIndex) => {
                ...
                rows += `<tr ${rowStyle} onclick="this.classList.toggle('active-row')"><td class="col-idx">${globalIdx++}</td>${imgCellHtml}<td class="col-name">${i.name}</td><td class="col-sku ${showSku ? '' : 'hidden-col'}">${i.displaySku}</td><td class="col-brand">${i.brand || 'STOUT'}</td><td class="col-unit">${i.unit || 'шт'}</td><td class="col-qty">${qHtml}</td><td class="col-price">${i.price.toLocaleString()}<span class="mob-mult" style="display:none;"> × ${i.q}</span></td><td class="col-sum">${i.sum.toLocaleString()}</td></tr>`;
            });
        ...
        const flushWorks = () => {
            ...
            for (let g of sortedGroups) {
                ...
                worksByGroup[g].forEach((w, idx) => {
                    ...
                    rows += `<tr ${rowStyle} onclick="this.classList.toggle('active-row')"><td class="col-idx">${globalIdx++}</td><td class="col-img hidden-col"></td><td class="col-name"><span class="work-del-btn" onclick="event.stopPropagation(); app.deleteWork('${w.name}')" title="Удалить работу">✖</span>${w.name}</td><td class="col-sku ${showSku ? '' : 'hidden-col'}">-</td><td class="col-brand hidden-col"></td><td class="col-unit">${w.unit}</td><td class="col-qty"><div class="qty-wrap">${w.q}</div></td><td class="col-price"><span class="price-edit" contenteditable="true" onblur="app.updateWorkPrice('${w.name}', this.innerText)" title="Изменить цену">${w.price.toLocaleString()}</span><span class="mob-mult" style="display:none;"> × ${w.q}</span></td><td class="col-sum">${w.sum.toLocaleString()}</td></tr>`;
                });
            ...
            // Кнопка добавления новой работы в самом низу
            let btnClass = (!this.state.tgUser || this.state.accountType !== 'pro') ? "btn-add-zone locked-pro" : "btn-add-zone";
            h += `<tr class="hide-custom-work-btn" style="background: transparent; border: none;"><td colspan="9" style="text-align: center; padding: 25px 0;"><button class="${btnClass}" style="max-width:300px; margin:0 auto; font-size: 14px; padding: 12px;" onclick="app.addCustomWork()">+ Добавить свою работу</button></td></tr>`;
        };
        ...
        if (this.state.hotWater) {
            ...
            let volByRes = this.state.res >= 7 ? 300 : this.state.res >= 5 ? 200 : this.state.res >= 3 ? 150 : 100;
            ...
            let warn = hw_fixtures_vol > 300 ? `<br><b style="color:#EF4444; font-size:10px;">⚠️ Внимание:</b> Пиковый расход ГВС (${hw_fixtures_vol} л) превышает макс. объем бойлера (300 л). При одновременном включении всех ванн и душей возможна нехватка горячей воды.` : "";
        ...
        if (hasRad && radSecs > 0) {
            ...
            if (this.state.detailedRooms && this.state.rooms && this.state.rooms.length > 0) {
                this.state.rooms.forEach(r => {
                    ...
                    r.windows.forEach((w, wIdx) => {
                        ...
                        if (w.isPan) {
                            ...
                            let reqSecs = Math.max(4, Math.ceil(wLoad / 117));
                            if ((reqSecs * 0.08) < w.width * 0.7) reqSecs = Math.max(reqSecs, Math.ceil((w.width * 0.7) / 0.08));
                            if (reqSecs > 14) reqSecs = 14;

                            let bestPanel = steelRads.find(s => s.power50 >= wLoad && (s.sec / 1000) >= w.width * 0.7);
                            if (!bestPanel) bestPanel = steelRads.find(s => s.power50 >= wLoad) || steelRads[steelRads.length - 1];

                            let itemSpace = catalog.rads.find(x => x.sec === reqSecs) || catalog.rads[catalog.rads.length - 1];
                            let itemTitan = titanRads.find(x => x.sec === reqSecs) || titanRads[titanRads.length - 1];
                            let altsList = [itemSpace, itemTitan, bestPanel]; itemSpace.alts = altsList; itemTitan.alts = altsList; bestPanel.alts = altsList;

                            let activeItem = itemSpace;
                            let factPower = reqSecs * 117;
                            let gostPower = reqSecs * 181;

                            if (this.state.radType === 'titan') {
                                activeItem = itemTitan; factPower = reqSecs * 128; gostPower = reqSecs * 198;
                            } else if (this.state.radType === 'steel') {
                                activeItem = bestPanel; factPower = bestPanel.power50; gostPower = Math.round(bestPanel.power50 / 0.65);
                            }

                            if (factPower < Math.round(wLoad)) {
                                app.tempWarns.push(`• <b>${r.name} (Окно ${wIdx + 1}):</b> дефицит мощности радиатора ~${Math.round(wLoad) - factPower} Вт.`);
                            }

                            let devInfo = this.getDesc('rad_item_detailed', gostPower, factPower);
                ...
                let win = this.state.win; let loadPerWindow = heatLoadTotal / win;
                let totalSecSpace = Math.ceil(heatLoadTotal / 117); let countSpace = Math.max(win, Math.ceil(totalSecSpace / 14)); let secPerRadSpace = Math.max(4, Math.min(14, Math.ceil(totalSecSpace / countSpace))); let itemSpace = catalog.rads.find(x => x.sec === secPerRadSpace) || catalog.rads[catalog.rads.length - 1];
                let totalSecTitan = Math.ceil(heatLoadTotal / 128); let countTitan = Math.max(win, Math.ceil(totalSecTitan / 14)); let secPerRadTitan = Math.max(4, Math.min(14, Math.ceil(totalSecTitan / countTitan))); let itemTitan = titanRads.find(x => x.sec === secPerRadTitan);
                let bestPanel = steelRads.find(s => s.power50 >= loadPerWindow); let countSteel = win; if (!bestPanel) { let maxPanel = steelRads[steelRads.length - 1]; countSteel = Math.max(win, Math.ceil(heatLoadTotal / maxPanel.power50)); bestPanel = maxPanel; } let itemSteel = bestPanel;
                let altsList = [itemSpace, itemTitan, itemSteel]; itemSpace.alts = altsList; itemTitan.alts = altsList; itemSteel.alts = altsList;
                let activeItem = itemSpace; totalRadCount = countSpace;
                if (this.state.radType === 'titan') { activeItem = itemTitan; totalRadCount = countTitan; } else if (this.state.radType === 'steel') { activeItem = itemSteel; totalRadCount = countSteel; }
                addToBill(activeItem, totalRadCount, this.getDesc('rad_item', heatLoadTotal), "3. Приборы отопления");
            }
        ...
        if (this.isAppReady) {
            ...
            this.hasUnsavedChanges = (this.lastSavedStateString !== this.getStateSignature());
            this.updateSaveBtnUI();
        }
    },
};
