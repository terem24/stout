...
const app = {
    ...
    showModal: function (type) {
        ...
        if (type === 'guest') {
            ...
            text.innerHTML = "Авторизуйтесь через Email, Google, чтобы получить доступ к этой функции.";
            ...
            icon.innerHTML = "⭐️";
            title.innerHTML = "Подписка PRO";
            text.innerHTML = "Профессиональные функции: монтажные работы, водоснабжение и артикулы.";
            ...
            if (demoBtn) {
                if (!this.state.demoUsed) {
                    ...
                    text.innerHTML += "<br><br><b>Или активируйте бесплатный тестовый период!</b>";
                } else {
    ...
    activateTrial: function () {
        // Текущее время + 7 дней в мс
        const trialUntil = Date.now() + 7 * 24 * 60 * 60 * 1000;
        localStorage.setItem('pro_trial_until', trialUntil);
        
        this.closeModal();
        alert('✅ Тестовый период на 7 дней успешно активирован!');
        
        // Перезагружаем страницу для полного обновления всех доступов и интерфейса
    ...
    showProForm: function (planName) {
        const container = document.getElementById('pro_form_block');
        if (!container) return;

        const user = this.state.user || this.state.tgUser || {};
        
        container.innerHTML = `
            <div class="modal-subscription" style="
            ...
            ">
                <h2 style="
                    ...
                    color: #000;
                    line-height: 1.3;
                ">Войдите, чтобы оформить подписку</h2>
                
                <input type="hidden" id="sub_plan" value="${planName}">
                
                <input type="text" id="sub_name" placeholder="Имя и фамилия" value="${user.name || user.first_name || ''}" 
                    style="width: 80%; height: 55px; border: 1px solid #e5e5e5; border-radius: 10px; padding: 0; margin: 0 auto 12px auto; font-size: 16px; text-align: center; background: #fff; box-sizing: border-box; display: block;">
                    
                <input type="email" id="sub_email" placeholder="Электронная почта" value="${user.email || ''}" 
                    style="width: 80%; height: 55px; border: 1px solid #e5e5e5; border-radius: 10px; padding: 0; margin: 0 auto 12px auto; font-size: 16px; text-align: center; background: #fff; box-sizing: border-box; display: block;">
                    
                <input type="tel" id="sub_phone" placeholder="Номер телефона" value="${user.phone || ''}" 
                    style="width: 80%; height: 55px; border: 1px solid #e5e5e5; border-radius: 10px; padding: 0; margin: 0 auto 25px auto; font-size: 16px; text-align: center; background: #fff; box-sizing: border-box; display: block;">
                    
                <button onclick="app.sendSubscriptionRequest()" id="btn_pro_submit" style="
                    width: 80%; 
                    height: 55px; 
                    background: #00A859; 
                    color: #fff; 
                ...
                <div style="margin-top: 20px;">
                    <a href="#" onclick="app.closeModal()" style="color: #007bff; text-decoration: none; font-size: 15px;">Закрыть</a>
                </div>
            </div>
        `;

        document.getElementById('pro_main_content').style.display = 'none';
        container.style.display = 'block';
    },

    showProCards: function () {
        document.getElementById('pro_main_content').style.display = 'block';
        document.getElementById('pro_form_block').style.display = 'none';
        let msg = document.getElementById('pro_success_message');
        if (msg) msg.style.display = 'none';
    },

    sendSubscriptionRequest: async function () {
        const btn = document.getElementById('btn_pro_submit');
        const originalText = btn.innerText;
        btn.innerText = "Отправка...";
        btn.disabled = true;

        const templateParams = {
            selected_plan: document.getElementById('sub_plan').value,
            user_name: document.getElementById('sub_name').value,
            user_email: document.getElementById('sub_email').value,
            user_phone: document.getElementById('sub_phone').value
        };

        try {
            const EMAILJS_SERVICE_ID = "service_o11b4ej";
            const EMAILJS_TEMPLATE_ID = "template_ysuxfio";
            ...
            if (res.status === 200) {
                ...
                container.innerHTML = `
                    <div style="text-align: center; padding: 40px; background: #fff; border-radius: 12px;">
                        ...
                        <h2 style="font-size: 22px; font-weight: 700; margin-bottom: 10px; color: #000;">Заявка отправлена!</h2>
                        <p style="color: #666; font-size: 14px; margin-bottom: 25px;">Мы свяжемся с вами в ближайшее время для активации доступа.</p>
                        <button onclick="app.closeModal()" style="width: 100%; height: 50px; background: #f3f4f6; color: #111; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">Закрыть</button>
                    </div>
                `;
            } else {
                throw new Error("Ошибка сервиса: " + res.text);
            }
        } catch (error) {
            console.error("EmailJS Error:", error);
            alert("⚠️ Ошибка EmailJS при отправке заявки:\n" + JSON.stringify(error));
    ...
    saveToCloud: async function (silent = false) {
        ...
        try {
            ...
            if (!dbUserId && !isLocal) {
                alert("Ошибка: профиль пользователя не найден. Попробуйте перезайти в аккаунт.");
                return false;
            }
    ...
    loadFromCloudList: async function () {
        ...
        if (!session && !isLocal) {
            alert("Для просмотра сохраненных смет необходимо авторизоваться.");
            this.showAuthModal();
            return [];
        }
        ...
        try {
            ...
            if (!uRow && !isLocal) {
                alert("У вас пока нет сохраненных смет.");
                return [];
            }
            ...
            if (userEmail === 'dima24ba@gmail.com') {
                // ИИ: Админ видит все сметы без ограничений
            } else if (uRow) {
                query = query.eq('user_id', uRow.id);
            } else if (isLocal) {
                // В режиме разработки на localhost без пользователя можно оставить без фильтра для удобства,
                // но RLS на стороне БД все равно должен защищать данные.
                // Однако по требованию "строго фильтровала" - добавим фильтр если есть хоть какой-то ID
                if (this.state.tgUser && this.state.tgUser.id) query = query.eq('user_id', this.state.tgUser.id);
            }
    ...
    handleGoogleAuth: async function (session) {
        ...
        try {
            ...
            if (uRow) {
                ...
                // Проверка: если город не указан — просим заполнить профиль
                if (!uRow.city) {
                    setTimeout(() => {
                        this.showProfileModal();
                        alert("Пожалуйста, укажите ваш город. Это необходимо для корректного формирования счетов.");
                    }, 1000);
                }
            } else {
    ...
    sendEmail: async function () {
        ...
        try {
            ...
            // ШАГ А: Сохранение и синхронизация ID с базой данных
            btn.innerHTML = '⌛ Сохранение и синхронизация...';
    ...
    init: function () {
        ...
        if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
            console.warn('[DEV MODE] Localhost detected — авторизация отключена, установлена сессия PRO.');
            this.state.tgUser = { id: 0, first_name: 'Dev', last_name: 'Localhost', username: 'dev_local' };
            this.state.accountType = 'pro';
        }
        // ===================================================
        this.syncUI(); this.render();
    ...
    syncUI: function () {
        ...
        if (cw && chk && sl) {
            ...
            if (!isAuthenticated) {
                ...
                let trialUntil = parseInt(localStorage.getItem('pro_trial_until')) || 0;
                let isTrialActive = trialUntil > Date.now();
                let isPro = (this.state.accountType === 'pro' || this.state.accountType === 'PRO' || isTrialActive);
        ...
        if (authContainer) {
            ...
            if (tgUser) {
                ...
                let isActuallyPro = accType === 'pro' || isTrialActive;
        ...
        const isPro = this.state.accountType === 'pro' || isTrialActive;
        ...
        const applyLock = (elId, reqLvl) => {
            ...
            if (elId === 'chk_dark' || elId === 'chk_water' || elId === 'chk_water_input' || elId === 'chk_well' || elId === 'chk_sku' || elId === 'chk_merge' || elId === 'chk_detailed_rooms' || elId === 'chk_scheme') {
    ...
    toggleSys: function (s, event) { 
        if (s === 'tp' && !this.checkAccess('pro', event)) return;
        setTimeout(() => {
