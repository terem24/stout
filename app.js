...
const app = {
    ...
    init: function () {
        ...
        // Скрываем прелоадер сразу после инициализации
    ...
    syncUI: function () {
        ...
        if (cw && chk && cl && sl) {
            ...
            // 2. Проверяем тариф
            let tgAcc = this.state.tgUser ? this.state.tgUser.account_type : null;
            let isPro = (this.state.accountType === 'pro' || this.state.accountType === 'PRO' || tgAcc === 'pro' || tgAcc === 'admin');

            if (!isAuthenticated || !isPro) {
                // Если вообще нет авторизации или базовый тариф — скрываем блок полностью
                ...
                cw.style.display = 'flex'; // Показываем блок только для PRO и ADMIN
