...
const app = {
    ...
    showProForm: function (planName) {
        ...
        if (form) {
            ...
            const user = this.state.tgUser || this.state.user || {};
            form.user_name.value = user.first_name || user.username || user.name || "";
            form.user_email.value = user.email || "";
            form.user_phone.value = user.phone || "";
        }
    },
    ...
    renderAdminMain: function () {
        ...
        users.forEach((u, i) => {
            ...
            let searchStr = `${name} ${phone} ${u.email || ''} ${u.tg_username || ''} ${cityText} ${ipLoc}`.toLowerCase();
                    ...
                    <div style="background: var(--surface-light); padding: 25px; border-radius: 16px; border: 1px solid var(--border); box-shadow: 0 4px 20px rgba(0,0,0,0.05); margin-bottom: 30px;">
                        <div style="display:flex; align-items:center; gap:20px; margin-bottom:25px;">
                            ...
                            <div>
                                ...
                                <div class="user-location-details" style="margin-top: 5px; font-size: 14px;">
                                    <div style="margin-bottom: 4px; color: var(--text-main);">
                                        📍 <span style="opacity: 0.7; font-size: 12px;">Город (регистрация):</span> <b>${user.city || 'Не указан'}</b>
                                    </div>
                                    <div style="color: var(--text-sec); font-size: 13px;">
                                        🌐 <span style="opacity: 0.7; font-size: 11px;">По IP:</span> ${user.location || 'Нет данных'}
                                    </div>
                                </div>
