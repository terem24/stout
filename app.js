...
const app = {
    ...
    toggleWaterInput: function (val) {
        ...
        let optBlock = document.getElementById('blk_water_input_opts');
        if (optBlock) {
            if (val) {
                optBlock.classList.add('open');
            } else {
                optBlock.classList.remove('open');
            }
        }
    ...
    init: function () {
        ...
        // ===================================================
        // Восстановить видимость блока подопций Узла ввода ХВС из state
        let optBlock = document.getElementById('blk_water_input_opts');
        if (optBlock) optBlock.classList.toggle('open', !!this.state.waterInput);
        this.syncUI(); this.render();
        ...
        if (this.state.water && this.state.waterZones.length > 0) {
            ...
            if (this.state.waterInput) {
                ...
                if (this.state.bigBlueFilter) {
                    ...
                    addToBill(bb[0], 1, "", grp63); // Колба
                    addToBill(bb[1], 1, "", grp63); // Картридж
                    addToBill(bb[2], 3, "", grp63); // Кран x3
                    addToBill(bb[3], 1, "", grp63); // Удлинитель 80мм
                    addToBill(bb[4], 2, "", grp63); // Удлинитель 30мм x2
                    addToBill(bb[5], 2, "", grp63); // Тройник x2
                    addToBill(bb[6], 3, "", grp63); // Ниппель x3
                    addToBill(bb[7], 2, "", grp63); // Угольник x2
                    addToBill(bb[8], 2, "", grp63); // Сгон угловой x2
                    addToBill(bb[9], 2, "", grp63); // Ниппель 1"x3/4" x2
                }
            ...
            if (totalToilets > 0) {
                ...
                addToBill(catalog.water_parts[6], totalToilets, this.getDesc('install'), "8. Канализация");
