...
const app = {
    ...
    cycleSwap: function (originalId) {
        // 1. Поиск товара в каталоге для проверки наличия списка замен (alts)
        let baseItem = null;
        for (let k in catalog) {
            if (Array.isArray(catalog[k])) {
                baseItem = catalog[k].find(x => x.id === originalId);
                if (baseItem) break;
            }
        }

        // 2. Если у товара или его аналога прописаны alts — используем их для переключения
        if (baseItem) {
            let alts = (this.state.brandMode === 'rommer' && baseItem.rommer) ? baseItem.rommer.alts : baseItem.alts;
            if (alts && alts.length > 0) {
                let currentId = (this.state.swaps && this.state.swaps[originalId]) || (this.state.brandMode === 'rommer' && baseItem.rommer ? baseItem.rommer.id : baseItem.id);
                let idx = alts.findIndex(a => a.id === currentId);
                let nextIdx = (idx + 1) % alts.length;
                if (!this.state.swaps) this.state.swaps = {};
                this.state.swaps[originalId] = alts[nextIdx].id;
                this.state.showSwapFor = null;
                this.render();
                return;
            }
        }

        // 3. Старая логика (Жестко прописанные переключения глобальных типов)
    ...
    render: function () {
        ...
        const addToBill = (item, qty, tip, group = null) => {
            ...
            if (this.state.brandMode === 'rommer' && (item.rommer || ANALOG_MAP[item.id])) {
                if (item.rommer && Array.isArray(item.rommer)) {
                    ...
                    item.rommer.forEach((sub, idx) => {
                        let finalSub = { ...sub };
                        finalSub.brand = sub.brand || "ROMMER"; // Явно прописываем бренд, если не указан
                        itemsToAdd.push({ itm: finalSub, q: qty, basePrice: (idx === 0 ? (item.price || 0) : 0) });
                    });
                } else {
                    // Одиночный аналог
                    let finalItem = { ...item };
                    ...
                    // ЕСЛИ ЕСТЬ ВЫБРАННЫЙ СВОП (Замена вручную пользователем)
                    if (this.state.swaps && this.state.swaps[item.id]) {
                        ...
                        if (found) {
                            // Если мы в режиме Rommer, пытаемся найти аналог для выбранного через своп товара
                            if (this.state.brandMode === 'rommer') {
                                let swappedAnalog = found.rommer;
                                if (!swappedAnalog && ANALOG_MAP[found.id]) {
                                    let targetId = ANALOG_MAP[found.id];
                                    for (let catKey in catalog) {
                                        if (Array.isArray(catalog[catKey])) {
                                            let a = catalog[catKey].find(x => x.id === targetId);
                                            if (a) { swappedAnalog = a; break; }
                                        }
                                    }
                                }
                                analog = swappedAnalog || found;
                            } else {
                                analog = found;
                            }
                        }
                    ...
                    // Если аналога нет в объекте и своп не переопределил его, ищем в глобальном маппинге
                    if (!analog && !this.state.swaps[item.id] && ANALOG_MAP[item.id]) {
                        // Ищем объект в каталоге
                    ...
                    if (analog) {
                        ...
                        itemsToAdd.push({ itm: finalItem, q: qty, basePrice: (item.price || 0) });
                    } else {
                        // Если аналог не найден в каталоге, добавляем оригинал
                        itemsToAdd.push({ itm: { ...item }, q: qty, basePrice: (item.price || 0) });
                    }
                }
            } else {
                itemsToAdd.push({ itm: { ...item }, q: qty, basePrice: (item.price || 0) });
            ...
            itemsToAdd.forEach(entry => {
                ...
                if (!this.state.optItems[finalItem.id]) {
                    this.calcFinalTotal += (finalItem.price || 0) * finalQty;
                    this.calcBaseTotal += (entry.basePrice || 0) * finalQty;
                }
        ...
        const flushBill = (title, warn) => {
            ...
            bill.forEach((i, arrIndex) => {
                ...
                let nameClass = (hasAlts && this.state.brandMode === 'rommer') ? "col-name swappable-cursor" : "col-name";
                let nameClick = (hasAlts && this.state.brandMode === 'rommer') ? `onclick="event.stopPropagation(); app.cycleSwap('${lookupId}')" title="Нажмите, чтобы заменить"` : "";
        ...
        // Выбор схемы: узел подмеса на коллекторе (до 150 м2 ТП) или насосные группы в котельной
        let useMixingUnit = (hasTp && tpArea <= 150);
        ...
        if (hasTp && tpArea > 0) {
            // Если используем узел подмеса на коллекторе, в котельной группы ТП не ставим
            tQ = useMixingUnit ? 0 : (estMans > 0 ? estMans : 1);
        }

        // Коллектор и гидрострелка нужны, если общих групп быстрого монтажа >= 2
        ...
        if (hasTp && tpMeters > 0) {
            ...
            const proc = (a, lbl) => {
                ...
                for (let i = 0; i < n; i++) { let sz = Math.floor(l / n) + (i < (l % n) ? 1 : 0); let m = catalog.manifolds.find(x => x.loops === sz); if (m) { addToBill({ ...m, name: `Коллектор ТП ${sz} вых (${lbl})` }, 1, this.getDesc('manifold', sz, 'ufh')); mans++; if (useMixingUnit) { addToBill(catalog.mixing_units[0], 1, this.getDesc('ufh_mix')); addToBill(catalog.pumps_mix[0], 1, this.getDesc('pump_std')); } } }
            };
        ...
        if (needCollector) {
            ...
            if (rQ > 0) addToWorks("Монтаж насосной группы (радиаторы)", rQ, 6500, "шт", wGroup);
            if (tQ > 0) addToWorks("Монтаж насосной группы (тёплый пол)", tQ, 6500, "шт", wGroup);
        } else if (this.state.systems.includes('tp') && !useMixingUnit) {
        ...
        if (this.state.systems.includes('tp') && typeof tpArea !== 'undefined' && tpArea > 0) {
            ...
            // Если используется узел подмеса на коллекторе (локальный подмес)
            if (typeof useMixingUnit !== 'undefined' && useMixingUnit) {
                addToWorks("Сборка и установка узла подмеса", mans, 6000, "шт", tpGroup);
                addToWorks("Установка насоса", mans, 3000, "шт", tpGroup);
            }
        ...
        if (dBadge) {
            if (this.state.brandMode === 'rommer' && this.calcBaseTotal > this.calcFinalTotal) {
                ...
                let percent = (diff / this.calcBaseTotal) * 100;
                // Если экономия меньше 10%, показываем один знак после запятой
                let percentStr = percent < 10 ? (Math.round(percent * 10) / 10) : Math.round(percent);
                dBadge.textContent = 'Экономия ' + percentStr + '%';
