...
const ANALOG_MAP = {
    ...
    "SPC-0011-2560180": "RCP-0004-2560180",
    "SPC-0010-2560180": "RCP-0004-2560180",
    "SPC-0002-2560180": "RCP-0030-2560180",
    "SPC-0003-2560180": "RCP-0030-2560180"
};



const app = {
    state: { waterInput: false, convConnectionType: 'straight', detailedRooms: false, rooms: [], convectorType: 'scq', well: false, wellDepth: 30, wellDist: 15, wellAutoType: 'sirio', h1: 2.7, h2: 2.7, viewMode: 'equipment', showScheme: false, optItems: {}, darkMode: false, area: 150, floors: 1, region: 100, mat: 1.0, fuels: ['el'], systems: [], hotWater: false, recirc: false, res: 3, win: 4, tp1: 0, tp2: 0, showSku: false, coolant: 'water', groupItems: false, collapsedGroups: [], swaps: {}, showSwapFor: null, radType: 'space', headType: 'gas', connectionType: 'angled', boilerType: 'optibase', ufhZones: 1, ufhCtrl: 'mech', pumpType: 'default', boilerSeries: 'status', hydroType: 'combo', pipeType: 'insulated', ufhBaseType: 'mat', radManifoldType: 'standard', water: false, waterZones: [], ufhAuto: false, projectName: "", brandMode: "stout", customWorks: {} },
    ...
    activateDemo: async function () {
        ...
        try {
            ...
            else { alert("Ошибка: не удалось определить аккаунт. Войдите заново."); return; }
    ...
    saveToCloud: async function (silent = false) {
        ...
        try {
            let dbUserId = null;
            if (tgUser) {
                // Для всех пользователей Supabase Auth (Google + Magic Link)
                // используем auth_user_id — UUID из Supabase Auth, он всегда уникален.
                if (tgUser.authUserId) {
                    ...
                    // Если пользователь не найден — он будет создан при следующем handleGoogleAuth.
                    // Не делаем insert здесь, чтобы избежать дублей при race condition.
                } else if (tgUser.email) {
                    // Запасной путь: поиск по email
                    let { data: uData } = await supabaseClient.from('users').select('id').eq('email', tgUser.email).maybeSingle();
            ...
            const insertData = {
                project_name: pName,
                calc_id: this.state.calc_id || null,
            ...
            this.markAsSaved();
            if (!silent) alert("✅ Смета успешно сохранена!");
            return true;
        } catch (error) {
        ...
        try {
            ...
            if (tgUser) {
                ...
                if (tgUser.authUserId) {
                    ...
                    let { data } = await supabaseClient.from('users').select('id').eq('email', tgUser.email).maybeSingle();
                    uRow = data;
                }
                if (uRow) query = query.eq('user_id', uRow.id);
    ...
    handleGoogleAuth: async function (session) {
        ...
        // Защита от race condition: если уже обрабатываем сессию — выходим.
        // Это случается, когда onAuthStateChange + getSession стреляют одновременно.
        if (this._authHandling) return;
        this._authHandling = true;

        try {
            let user = session.user;
            let authUserId = user.id; // UUID из Supabase Auth — ГАРАНТИРОВАННО УНИКАЛЕН
            ...
            // Сохраняем в состоянии authUserId как главный ключ идентификации
            this.state.tgUser = {
                id: authUserId,         // оставляем для совместимости со старым кодом
                authUserId: authUserId, // явный ключ для новых запросов
                first_name: fullName,
                phone: phone,
                email: email,
                avatar_url: avatar,
            ...
            // Поля для обновления при каждом входе (визит, устройство, аватар)
            let updatePayload = {
                last_visited: new Date().toISOString(),
                last_device: this.getDeviceName(),
                avatar_url: avatar,
                location: loc,
                email: email  // обновляем email на случай смены
            };

            // UPSERT по auth_user_id — атомарная операция:
            // - если запись с таким auth_user_id уже есть → обновляет указанные поля
            // - если нет → создаёт новую запись
            // Условие onConflict: 'auth_user_id' требует, чтобы в таблице users
            // был UNIQUE constraint или Primary Key на колонке auth_user_id.
            let upsertObj = {
            ...
            // Убираем undefined-поля, чтобы не перезаписывать существующие значения NULL
            ...
            let { data: upsertResult, error: upsertError } = await supabaseClient
                ...
                .select('id, account_type, demo_ends_at, username, phone');

            if (upsertError) {
                // Если upsert не поддерживается (нет constraint) — fallback на select+update
                ...
                if (!uData) {
                    // Гарантируем одну запись: вставляем только если не нашли по email
                    let { data: newUList } = await supabaseClient
                        .from('users')
                        .insert([{ auth_user_id: authUserId, email: email, username: fullName, phone: phone, utm_source: utm, ...updatePayload }])
                        .select('id, account_type, demo_ends_at, username, phone');
            ...
            if (uRow) {
                ...
                if (demoEnds) {
                    ...
                    if (accType === 'pro' && new Date() > new Date(demoEnds)) {
                        accType = 'base';
                        // Обновляем в фоне, не блокируем UI
                        supabaseClient.from('users').update({ account_type: 'base' }).eq('auth_user_id', authUserId);
                    }
                }
                this.state.accountType = accType;
                // Берём имя и телефон из БД если они были сохранены пользователем вручную
            ...
            // Снимаем флаг блокировки в любом случае (через 2 сек, чтобы перекрыть двойной вызов)
    ...
    sendEmail: async function () {
        ...
        if (!tgUser) {
            alert("Пожалуйста, авторизуйтесь, чтобы отправить смету.");
            this.showAuthModal();
            return;
        }

        if (!tgUser.first_name || !tgUser.phone || tgUser.phone.length < 16) {
            alert("Пожалуйста, укажите Ваше Имя и Телефон в профиле. Они необходимы для отправки сметы.");
            this.showProfileModal();
            return;
        }
        ...
        try {
            // Шаг А: Сохранение в базу (оставляем без изменений)
            btn.innerHTML = '⌛ Сохранение...';
            btn.disabled = true;
            
            const isSaved = await this.saveToCloud(true); 
            if (!isSaved) {
            ...
            // ШАГ Б: Отправка через EmailJS (для статических сайтов без PHP)
            btn.innerHTML = '📧 Отправка почты...';
            this.render(); // Актуализируем суммы и спецификацию
            ...
            // РЕАЛЬНЫЕ ID из личного кабинета EmailJS
            ...
            const total = (app.lastEqSum || 0) + ((this.state.accountType === 'pro') ? (app.lastWorksSum || 0) : 0);
            ...
            // Отправка через EmailJS SDK
            const result = await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams);
            
            if (result.status === 200) {
                alert("✅ Счет успешно отправлен на почту!");
            } else {
                throw new Error("Сервер EmailJS вернул ошибку: " + result.text);
            }
            
        } catch (error) {
            console.error("Email send error:", error);
            let errorMsg = "Не удалось отправить письмо.";
            if (error && error.text) errorMsg += " " + error.text;
            else if (error && error.message) errorMsg += " " + error.message;
            
            alert("⚠️ Смета сохранена в облаке, но возникла ошибка при отправке Email:\n\n" + errorMsg);
        } finally {
            btn.innerHTML = originalHtml;
            btn.disabled = false;
        }
    ...
    loadFromCode: async function () {
        ...
        if (code.startsWith('HC-')) {
            try {
                ...
                if (data.calc_data) {
                    ...
                    this.syncUI();
                    this.render();
                    alert("✅ Расчет успешно загружен по коду!");
                    return;
                }
            } catch (err) {
                console.error("Ошибка загрузки по коду:", err);
                alert("❌ Ошибка! Неверный код или расчет не найден.");
                return;
            }
        }
        ...
        try {
            ...
            alert("❌ Ошибка! Неверный формат кода. Попробуйте скопировать снова.");
        }
    },
    ...
    init: function () {
        ...
        let r0 = catalog.rads[0], t0 = (typeof titanRads !== 'undefined' ? titanRads[0] : null), s0 = (typeof steelRads !== 'undefined' ? steelRads[0] : null);
        let radAlts = [r0, t0, s0].filter(x => x);
        catalog.rads.forEach(rad => { rad.alts = radAlts; }); 
        if (typeof titanRads !== 'undefined') titanRads.forEach(rad => { rad.alts = radAlts; }); 
        if (typeof steelRads !== 'undefined') steelRads.forEach(rad => { rad.alts = radAlts; });
        
        let hAlts = catalog.h_valves; catalog.h_valves.forEach(v => { v.alts = hAlts; });
        let boilerAlts = [catalog.tanks_optibase[0], catalog.tanks_standard[0]].filter(x => x); 
        catalog.tanks_optibase.forEach(t => { t.alts = boilerAlts; }); catalog.tanks_standard.forEach(t => { t.alts = boilerAlts; });
        
        let pumpAlts = catalog.pumps_dn25; catalog.pumps_dn25.forEach(p => { p.alts = pumpAlts; });
        let elBoilerAlts = [catalog.boilers_status[0], catalog.boilers_plus[0]].filter(x => x); 
        catalog.boilers_plus.forEach(b => { b.alts = elBoilerAlts; }); catalog.boilers_status.forEach(b => { b.alts = elBoilerAlts; });
        
        let hydroAlts = catalog.hydro_modular_dn20; 
        catalog.hydro_dn20.forEach(h => { h.alts = hydroAlts; }); catalog.hydro_modular_dn20.forEach(h => { h.alts = catalog.hydro_dn20; });
        
        let pipeAlts = catalog.rad_pipes_grey; 
        catalog.insulated_pipes.forEach(p => { p.alts = pipeAlts; }); catalog.rad_pipes_grey.forEach(p => { p.alts = catalog.insulated_pipes; });
        
        if (catalog.manifolds_rad_standard && catalog.manifolds_rad_chrome) { 
            let chromeAlt = catalog.manifolds_rad_chrome[0]; 
            catalog.manifolds_rad_standard.forEach(m => { m.alts = [chromeAlt]; }); 
            catalog.manifolds_rad_chrome.forEach(m => { m.alts = [catalog.manifolds_rad_standard[0]]; }); 
        }
        let xpsAlt = catalog.xps_kit ? catalog.xps_kit[0] : null; 
        if (xpsAlt) { catalog.mats.forEach(m => { m.alts = [xpsAlt]; }); xpsAlt.alts = catalog.mats; }
        if (catalog.well_auto) { let waAlts = catalog.well_auto; catalog.well_auto.forEach(a => { a.alts = waAlts; }); }

        // Безопасное копирование списков замен для Rommer-аналогов
        for (let k in catalog) {
            if (Array.isArray(catalog[k])) {
                catalog[k].forEach(item => {
                    if (item.alts && item.rommer) {
                        if (Array.isArray(item.rommer)) item.rommer.forEach(s => { if (typeof s === 'object') s.alts = item.alts; });
                        else item.rommer.alts = item.alts;
                    }
                });
            }
        }
        ...
        // ВАЖНО: onAuthStateChange может стрелять несколько раз при одном входе
        // (например TOKEN_REFRESHED + SIGNED_IN). Защита от race condition реализована
        // внутри handleGoogleAuth через флаг _authHandling.
        supabaseClient.auth.onAuthStateChange((event, session) => {
            if ((event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') && session) {
                // Обновляем данные при каждом входе и обновлении токена,
                // но внутренний флаг _authHandling блокирует параллельные вызовы.
        ...
        supabaseClient.auth.getSession().then(({ data: { session } }) => {
            if (session) {
                // Флаг ещё не установлен, поэтому этот вызов пройдёт первым.
                // Последующий вызов из onAuthStateChange будет заблокирован флагом.
                this.handleGoogleAuth(session);
            }
    ...
    cycleSwap: function (id) {
        let originalId = id;
        // Если передан ID Rommer, пытаемся найти соответствующий Stout ID через ANALOG_MAP (реверсивный поиск) или catalog
        if (id.startsWith('RCP-') || id.startsWith('RHT-') || id.startsWith('RBV-') || id.startsWith('RCA-')) {
            for (let stoutKey in ANALOG_MAP) {
                if (ANALOG_MAP[stoutKey] === id) { originalId = stoutKey; break; }
            }
            if (originalId === id) {
                for (let k in catalog) {
                    if (Array.isArray(catalog[k])) {
                        let stoutItem = catalog[k].find(x => x.rommer && (Array.isArray(x.rommer) ? x.rommer.some(s => s.id === id) : x.rommer.id === id));
                        if (stoutItem) { originalId = stoutItem.id; break; }
                    }
                }
            }
        }
        // Старая логика (Жестко прописанные переключения глобальных типов)
    ...
    // ====================================
    render: function () {
        const titanRads = (typeof window.titanRads !== 'undefined') ? window.titanRads : (catalog.rads_titan || []);
        const steelRads = (typeof window.steelRads !== 'undefined') ? window.steelRads : (catalog.rads_steel || []);
        let h = "";
        ...
        let pwr = 0, totalRadCount = 0, totalConvCount = 0, totalVartronic = 0, heatLoadTotal = 0, tpArea = 0, tpMeters = 0, mans = 0, loops = 0, finalCnt = 0, useEco = false, radSecs = 0, radMeters = 0, manifoldsCount = 0;
        if (this.state.detailedRooms && this.state.rooms && this.state.rooms.length > 0) {
            let totalLoadW = 0;
            this.state.rooms.forEach(r => {
        ...
        const addToBill = (item, qty, tip, group = null) => {
            ...
            // 1. ПРИОРИТЕТ: Ручная замена пользователем (Swaps)
            if (this.state.swaps && this.state.swaps[item.id]) {
                let swappedId = this.state.swaps[item.id];
                let found = null;
                for (let k in catalog) {
                    if (Array.isArray(catalog[k])) {
                        found = catalog[k].find(x => x.id === swappedId);
                        if (found) break;
                    }
                }
            ...
            // 2. БРЕНД: Переход на Rommer, если включен режим
            if (this.state.brandMode === 'rommer') {
                this.calcBaseTotal += (currentItem.price || 0) * qty;
                
                let analog = currentItem.rommer;
                if (!analog && ANALOG_MAP[currentItem.id]) {
                    let targetId = ANALOG_MAP[currentItem.id];
                    for (let catKey in catalog) {
                        if (Array.isArray(catalog[catKey])) {
                            let a = catalog[catKey].find(x => x.id === targetId);
                            if (a) { analog = a; break; }
                        }
                    }
                }
                ...
                if (analog) {
                    if (Array.isArray(analog)) {
                        ...
                        if (analog.alts) finalItem.alts = analog.alts;
                        itemsToAdd.push({ itm: finalItem, q: qty });
                    }
                } else {
                    itemsToAdd.push({ itm: currentItem, q: qty });
                }
            } else {
                // Режим STOUT
                this.calcBaseTotal += (currentItem.price || 0) * qty;
            ...
            itemsToAdd.forEach(entry => {
                ...
                if (forceMerge) {
                    ...
                    if (existing) {
                        ...
                        bill.push({ ...finalItem, q: finalQty, sum: finalItem.price * finalQty, displaySku: finalItem.article || finalItem.id, qtyTip: finalTip || "", group: group, originalId: finalItem.id });
                    }
                } else {
                    let finalTip = tip;
        ...
        const flushBill = (title, warn) => {
            ...
            bill.forEach((i, arrIndex) => {
                ...
                let hasAlts = (i.alts && i.alts.length > 0) || (this.state.swaps && this.state.swaps[lookupId]);
                let imgCellHtml = "";
                if (hasAlts) {
        ...
        radSecs = 0, radMeters = 0, tpMeters = 0;
        tpArea = this.state.tp1 + this.state.tp2;
        ...
        if (hasRad && radSecs > 0) {
            ...
            totalConvCount = 0;
            totalVartronic = 0;
            heatLoadTotal = Math.round((hasTp && tpArea > 0) ? pwr * 700 : pwr * 1000);
            ...
            if (totalRadCount > 0) {
                ...
                let activeHead = catalog.heads.find(h => h.type === this.state.headType) || catalog.heads[0]; addToBill(activeHead, totalRadCount, this.getDesc('rad_head'), grp);
                if (activeHead.type === 'smart') { let radHubs = Math.ceil(totalRadCount / 15); addToBill(catalog.smart_hub, radHubs, `Шлюз Zigbee.`, grp); }
                let activeHValve = catalog.h_valves.find(v => v.type === this.state.connectionType) || catalog.h_valves[0]; addToBill(activeHValve, totalRadCount, this.getDesc('rad_valves'), grp);
            ...
            if (neededPipe > 0) {
                if (this.state.pipeType === 'insulated') {
                    ...
                    let itemRed = catalog.insulated_pipes[0]; addToBill(itemRed, halfCoils, `Труба в красной изол.`, pipeGrp);
                    let itemBlue = catalog.insulated_pipes[1]; addToBill(itemBlue, halfCoils, `Труба в синей изол.`, pipeGrp);
                } else {
                    let grayItem = (neededPipe > 200) ? catalog.rad_pipes_grey[1] : catalog.rad_pipes_grey[0]; addToBill(grayItem, Math.ceil(neededPipe / grayItem.len), this.getDesc('rad_pipe', neededPipe), pipeGrp);
            ...
            let reqLoops = (this.state.floors === 2 ? Math.ceil(totalDevicesCount / 2) : totalDevicesCount); if (reqLoops > 12) reqLoops = 12; manifoldsCount = (this.state.floors === 2) ? 2 : 1;
            if (this.state.radManifoldType === 'standard') { let m = catalog.manifolds_rad.find(x => x.loops === reqLoops) || catalog.manifolds_rad[catalog.manifolds_rad.length - 1]; if (m) { addToBill(m, manifoldsCount, this.getDesc('manifold', totalDevicesCount, 'rad'), pipeGrp); } }
            else {
                const assemblyMap = { 2: [0, 0, 1], 3: [0, 1, 0], 4: [1, 0, 0], 5: [0, 1, 1], 6: [0, 2, 0], 7: [1, 1, 0], 8: [2, 0, 0], 9: [1, 1, 1], 10: [1, 2, 0], 11: [2, 1, 0], 12: [3, 0, 0] }; let plan = assemblyMap[reqLoops] || [3, 0, 0]; let b4 = catalog.manifolds_chrome_blocks[2]; let b3 = catalog.manifolds_chrome_blocks[1]; let b2 = catalog.manifolds_chrome_blocks[0]; let multiplier = manifoldsCount * 2;
        ...
        if (hasTp && tpMeters > 0) {
            ...
            if (this.state.ufhAuto) {
                ...
                finalCnt = Math.max(cntByZones, cntByFloors);
        ...
        if (this.state.systems.includes('rad') && typeof totalRadCount !== 'undefined' && totalRadCount > 0) {
            addToWorks("Монтаж трубопроводов PEX-a... и подключение радиатора", totalRadCount, 6500, "шт", radGroup);
