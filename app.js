...
const app = {
    ...
    renderScheme: function () {
        ...
        // Генерация HTML с обработчиком ошибок загрузки картинок
        let html = `<div id="dynamic_scheme" style="position: relative; width: 100%; height: 70vh; min-height: 400px; max-height: 800px; background: #fff; overflow: hidden;">`;
        layers.forEach(layer => {
            html += `<img src="${basePath}${layer}" alt="${layer}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: contain;" onerror="this.style.display='none'">`;
        });
        html += `</div>`;
        return html;
    },
