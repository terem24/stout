...
const RecognizeMatch = (function () {
  ...
  const NOT_OUR_RANGE = [
    ...
    /хомут[\s-]*стяжк/i,
    /**
     * Фланец плоский стальной по ГОСТ — тоже магистраль.
     *
     * «Фланец плоский стальной 20-16 01-1-B ГОСТ 33259-2015» — это фланец
     * трубопровода большого диаметра, у поставщика таких нет. Фланцы, которые
     * он возит, — другие и называются иначе: Thermaflex к предизолированной
     * трубе («Фланец 25мм»), крепление к котлам BAXI, гайка под фланец MEIBES.
     * Поэтому правило требует все три слова разом: фланец, плоский, стальной.
     */
    /^(?=[\s\S]*фланец)(?=[\s\S]*плоск)(?=[\s\S]*стальн)/i,
    /**
  ...
  const EQUIVALENTS = [
    ...
    /**
     * Обезжириватель резьбы: у поставщика он есть, просто зовут иначе.
     *
     * «Обезжириватель сантехмастергель, баллон 150 мл» не находил ничего, хотя
     * короткое «обезжириватель резьбы» подбирается: предметом строки
     * становится марка, и подбор теряется на ней. Слово «обезжириват»
     * однозначно, по нему и ставим. Объём другой — 650 мл против 150, — и об
     * этом сказано в пометке: это разные упаковки одного средства.
     */
    { test: /обезжириват/i, id: 'MB402700650',
      note: 'прямая замена: обезжириватель резьбы Mr.Bond 650 мл — объём другой, сверьте расход' },
  ...
  /**
   * Шток для хомута — это сантехническая шпилька.
   *
   * «Шток д/хомута 10*100 мм» не находил ничего: штоком её зовут в одних
   * счетах, шпилькой в других, а общего слова у них нет. Семь промахов.
   *
   * Резьба у наших шпилек только М8, в счетах попадается М10 — подменять
   * молча нельзя, поэтому строка приходит с пометкой. Длину берём вторым
   * числом и округляем вверх по ряду: короче нужного шпилька не годится.
   */
  function isStud(rec) {
    const raw = String(rec.raw || '').toLowerCase().trim();
    /**
     * Предметом строки должен быть сам шток, а не упоминание о нём.
     *
     * «Хомут 110-116мм 4 с гайкой без штока» — это хомут, и слово «штока» там
     * стоит как раз затем, чтобы сказать: шпильки в комплекте нет. Первая
     * редакция правила ловила и такие строки, и хомут за 30 ₽ подменялся
     * шпилькой за 48. Поэтому шток обязан стоять в начале строки.
     */
    if (/без\s+штока/.test(raw)) return false;
    return /^шток/.test(raw) && /хомут|шпильк|санте/.test(raw);
  }

  function matchStud(rec) {
    const pool = [];
    const add = (it) => {
      if (it && it.name && /шпильк[а-яё]*\s+сантехническ/i.test(it.name) &&
          !pool.some((x) => String(x.id) === String(it.id))) pool.push(it);
    };
    // Каталожная копия идёт первой: у неё своя цена и фото.
    for (const key in catalog) {
      const v = catalog[key];
      if (Array.isArray(v)) v.forEach(add);
    }
    for (const row of (priceIndex || [])) {
      add({ id: row.a, name: row.n, price: row.p, brand: 'STOUT' });
    }
    if (!pool.length) return null;

    const raw = String(rec.raw || '');
    // «10*100» — резьба и длина; берём второе число, оно и есть длина.
    const pair = raw.match(/(\d{1,2})\s*[*х x]\s*(\d{2,4})/i);
    const want = pair ? +pair[2] : (rec.d || 0);
    if (!want) return null;

    // «М8*100мм» в прайсе и «M8x100» в каталоге — буква М там русская, тут
    // латинская, разделитель то звёздочка, то икс. Ловим оба написания:
    // иначе каталожная копия остаётся без длины и выпадает из отбора.
    const len = (it) => {
      const m = String(it.name).match(/[мm]\s*8\s*[*хx×]\s*(\d{2,4})/i);
      return m ? +m[1] : 0;
    };
    const fit = pool.filter((it) => len(it) >= want).sort((a, b) => len(a) - len(b));
    const hit = fit[0] || pool.sort((a, b) => len(b) - len(a))[0];
    if (!hit) return null;

    return {
      item: hit,
      score: 0.85,
      substituted: 'шток заменён сантехнической шпилькой М8 — сверьте резьбу и длину',
      brandRank: brandRank(hit),
      needsApproval: true,
      alternatives: pool.filter((it) => it !== hit).slice(0, 3),
    };
